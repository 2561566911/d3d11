...
void CTabSheet::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (m_nCurrentPage != GetCurFocus())
	{
		...
		//		m_pPages[n_nCurrentPage]->SetFoeus();
	}
}

int CTabSheet::SetCurSel(int nIten)
{
	if( nIten < 0 || nIten >= m_nNumOfPages)
	return -1;

	int ret = m_nCurrentPage;

	if (m_nCurrentPage != nIten)
	{
		m_pPages[m_nCurrentPage]->ShowWindow(SW_HIDE);
		m_nCurrentPage = nIten;
		m_pPages[m_nCurrentPage]->ShowWindow(SW_SHOW);
		//		m_pPages[m_nCurrentPage]->SetFocus();
		CTabCtrl::SetCurSel(nIten);
	}
